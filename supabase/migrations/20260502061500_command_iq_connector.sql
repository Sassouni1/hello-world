...
create policy "Users manage their bridge devices"
  ...
  using (
    exists (
      ...
      where a.id = bridge_devices.account_id and a.owner_user_id = auth.uid()
    )
  )
  with check (
    exists (
...
create policy "Users manage their bridge sessions"
  ...
  using (
    exists (
      select 1 from public.bridge_accounts a
      where a.id = bridge_sessions.account_id and a.owner_user_id = auth.uid()
    )
  )
  with check (
    exists (
...
create policy "Users manage their bridge messages"
  ...
  using (
    exists (
      select 1 from public.bridge_accounts a
      where a.id = bridge_messages.account_id and a.owner_user_id = auth.uid()
    )
  )
  with check (
    exists (
...
create policy "Users manage their bridge commands"
  ...
  using (
    ...
    or exists (
      select 1 from public.bridge_accounts a
      where a.id = bridge_commands.account_id and a.owner_user_id = auth.uid()
    )
  )
  with check (
    requested_by = auth.uid()
...
create policy "Users manage their bridge pairing codes"
  ...
  using (
    exists (
      select 1 from public.bridge_accounts a
      where a.id = bridge_pairing_codes.account_id and a.owner_user_id = auth.uid()
    )
  )
  with check (
    exists (
